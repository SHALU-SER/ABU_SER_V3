...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('Abuser is Working😍 ' + config.WORKTYPE + ' 𝚗𝚘𝚠 🍃'));
          if (config.LANG == 'EN' || config.LANG == 'ML') {
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(conn.user.jid, fs.readFileSync("https://www.linkpicture.com/q/IMG-20220108-WA0143.jpg"), MessageType.image, { caption: `*ʜᴇʏ ʙʀᴏ ᴀʙᴜ_sᴇʀ_ᴠ3 ᴡᴏʀᴋɪɴɢ ` + config.WORKTYPE + ` 🥂* \n\n*ᴛʜᴀɴᴋʏᴏᴜ ғᴏʀ ᴜsɪɴɢ ᴀʙᴜ_sᴇʀ_ᴠ3🥂*`});
                } else {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '*◄━⦁💝 ABU_SER_V3💝⦁━►*' + degisiklikler + '```', MessageType.text
                    ); 
                } 
          }
    });//thanx afnanplk
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ 💖ABU_SER_V3💖 ~______~*' +
                                    '\n*ABU_SER ERROR🔰' +
                                    '\n\n*⚠️ ' + error + '*\n'
                                    , MessageType.text);
                            }
